...
fn stdio_returns_structured_errors_for_invalid_json_and_missing_save_selection() {
    ...
    assert_eq!(
        ...
        Some(AutomationTransportErrorCode::InvalidRequest)
    );
    assert_eq!(
        responses[1].error.as_ref().map(|error| error.code),
        Some(AutomationTransportErrorCode::CommandRejected)
    );
}

#[test]
fn stdio_returns_structured_errors_for_invalid_step_counts_and_promotion_without_context() {
    let root = tempdir().expect("temporary directory should be created");
    let mut harness =
        AutomationHarness::new(Some(root.path().to_path_buf())).with_semantic_automation();
    harness.boot_to_main_menu();
    ...
    let requests = [
        AutomationRequest {
            command: AutomationCommand::Step { frames: 0 },
        },
        AutomationRequest {
            command: AutomationCommand::Match(AutomationMatchAction::ChoosePromotion {
    ...
    let mut output = Vec::new();
    run_stdio_session(Cursor::new(input.into_bytes()), &mut output, &mut harness)
        .expect("transport should emit structured command errors");
